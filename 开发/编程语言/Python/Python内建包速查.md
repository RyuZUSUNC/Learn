...
# 字节处理(struct)
准确地讲，Python没有专门处理字节的数据类型。但由于·可以表示字节，所以，字节数组＝二进制str。而在C语言中，我们可以很方便地用struct、union来处理字节，以及字节和int，float的转换。
...
# os
## os.system('[cmd]')
- 其返回值是shell指令运行后返回的状态码，int类型，0表示shell指令成功执行，256/512表示未找到，该方法适用于shell命令不需要输出内容的场景。
