...
（ 译者注：关于 iota 的使用涉及到非常复杂多样的情况，这里作者解释的并不清晰，因为很难对 iota 的用法进行直观的文字描述。如希望进一步了解，请观看视频教程 [《Go编程基础》 第四课：常量与运算符](https://github.com/Unknwon/go-fundamental-programming/blob/master/lectures/lecture4.md) ）
...
另外，像`数组`（第 7 章）和`结构`（第 10 章）这些复合类型也是值类型。
...
示例 4.7 user_init.go 中导入了包 trans（需要init.go目录为./trans/init.go）并且使用到了变量 Pi：
...
个类型相同的值可以使用相等 `==` 或者不等 `!=` 运算符来进行比较并获得一个布尔型的值。
...
> 补码: 正数和0的补码就是该数字本身。负数的补码则是将其对应正数按位取反再加1。例如 8位 的补码可以表示 127 至 -128 总共256个整数。
...
## 修剪字符串
`strings.Fields(s)` 将会利用一个或多个空白符好来作为动态长度的分割符将字符串分割成若干个小块，并返回一个 slice，如果字符串只包含空白符号，则返回一个长度为 0 的 slice。
