...
![](img/distributed-runtime.svg)

## 基本的 stream source

上述示例用 `env.fromElements(...)` 方法构造 `DataStream<Person>` 。这样将简单的流放在一起是为了方便用于原型或测试。`StreamExecutionEnvironmen`t 上还有一个 `fromCollection(Collection)` 方法。因此，你可以这样做：
